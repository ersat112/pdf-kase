...
export type AssetLibraryOverviewItem = {
  key: string;
  title: string;
  subtitle: string;
  value: string | number;
...
type Props = {
  items: AssetLibraryOverviewItem[];
};

function getPalette(tone: AssetLibraryOverviewItem['tone']) {
  switch (tone) {
    case 'accent':
      return {
...
export function AssetLibraryOverviewStrip({ items }: Props) {
  if (!items.length) {
    return null;
  }
